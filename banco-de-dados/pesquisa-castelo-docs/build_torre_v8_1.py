"""Torre Check-In v8.1 — telhados corrigidos sem alterar corpo/escala.
...
def build_tower(cx, cz, ground_y=1):
    ...
    for i, (half, chamfer) in enumerate(green_profiles, start=1):
        ...
        fill_profile(cx, cz, y, half, chamfer,
                     G if i % 2 else DG)
        # O telhado baixo pede oito linhas: uma em cada quina do octógono.
        all_eave_tips(cx, cz, y, half, chamfer)
        if i == 1:
            shell_profile(cx, cz, y, half, chamfer, Q)
    ...
    for i, (half, chamfer) in enumerate(roof_profiles):
        ...
        fill_profile(cx, cz, y, half, chamfer,
                     G if i % 2 == 0 else DG)
        if half > 0:
            four_ribs(cx, cz, y, half, chamfer)
        if i == 0:
            shell_profile(cx, cz, y, half, chamfer, Q)
...
def main():
    ...
    legacy.world = world
    out = Path(__file__).with_name('torre-v8.1.schematic')
